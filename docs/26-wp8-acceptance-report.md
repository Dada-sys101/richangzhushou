...
> 后续状态（2026-08-07）：本报告完成后的发布决策已固化——正式产品名为
> “日常助手 / Daily Assistant”（OPEN-001），V1 通知范围仅应用内提醒（OPEN-005），
> 仓库名与技术标识保持 daily-assistant（OPEN-011）；浏览器 QA 已用 Playwright
> 自动化（OPEN-009，smoke/matrix 与 CI browser-qa）。本报告正文保留当时验收事实。

> 范围调整：WP9（docs/28）已下线邮箱/邀请码/截图 OCR，并将账号改为管理员创建。
> 本报告中的注册、邀请码、邮件恢复与 OCR 相关结论以 WP9 为准。
