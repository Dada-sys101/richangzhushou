...
| OPEN-006 | 部署地域与对象存储 | 代码已实现（OSS 适配器 + `STORAGE_PROVIDER` 切换 + `StorageKeyService`）；真实 Bucket/RAM/连通测试未完成，OPEN-006 仍未完成 |
...
> 尚需创建私有 Bucket、最小权限 RAM 凭据并完成真实上传/下载/删除与备份上传验证后才能关闭 OPEN-006。
