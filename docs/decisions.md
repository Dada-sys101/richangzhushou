...
| OPEN-006 | 部署地域与备案 | 代码已实现并经 PR #6 squash 合并到 main（`db5c5d3`，main CI 通过）；真实 Bucket、RAM 凭据与连通测试未完成；staging 未创建 | 生产上线 |
...
> PR #6 已 squash 合并到 main（`db5c5d3`），main CI 通过；真实 OSS 资源与连通验证待用户授权。
