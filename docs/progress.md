...
- 根 `npm run quality` 未全绿：dependency audit 仍 fail-closed；过期 `deepmerge-ts` 例外及 Prisma/MariaDB/MySQL2 相关高风险依赖尚未有兼容批准修复。
- 回滚后 `npm ci`、`npm ls`、治理测试 `14/14`、CycloneDX SBOM 生成/校验（1044 components）和 license inventory（1163 packages）PASS；本轮候选不计入最终依赖修复。
