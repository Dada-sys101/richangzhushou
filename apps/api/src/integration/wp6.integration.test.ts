...
describeWithDb(
  "WP6 trips, trip items, packing items, and expense summary",
  () => {
    ...
    it("packing items support position ordering, toggle, idempotency, and soft delete/restore", async () => {
      ...
      const afterDelete = await request(app.getHttpServer())
        ...
        .set("Authorization", `Bearer ${token}`);
      expect(
        afterDelete.body.packingItems.map(
    ...
    it("QA-SEC-001/002 extension: cross-user trip access is 404 and admins get 403", async () => {
      ...
      const checks: Array<{
        ...
        {
          method: "PATCH",
          path: `/api/v1/packing-items/${packingId}`,
          body: { version: 1 },
        },
    ...
    async function loginNewUser(): Promise<string> {
      const username = `wp6_${userSequence}_${Math.random()
        .toString(36)
        .slice(2, 8)}`;
      ...
      return login(username, TEST_PASSWORD).then(
