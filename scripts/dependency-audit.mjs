...
const EXCEPTION_STRING_KEYS = EXCEPTION_KEYS.filter(
  (key) => key !== "coveredFindings",
);
...
function hasExactStringArray(value, expected) {
  return (
    ...
    value.every(
      (entry, index) => isNonEmptyString(entry) && entry === expected[index],
    )
  );
}
...
export function validateExceptionConfig(config) {
  ...
  if (
    !hasExactStringArray(
      exception.coveredFindings,
      EXPECTED_EXCEPTION.coveredFindings,
    )
  ) {
    errors.push(
      "exception field coveredFindings must exactly cover deepmerge-ts, @prisma/config, and prisma",
    );
  }
  ...
  if (!/^CVE-\d{4}-\d{4,}$/u.test(exception.cve ?? "")) {
    errors.push("exception cve must be a canonical CVE identifier");
  }
  if (exception.severity !== "high") {
    errors.push("exception severity must be exactly high");
  }
...
export function validateNanoidResolution(packageLock) {
  const errors = [];
  ...
  const nanoidPaths = Object.keys(packageLock.packages).filter(
    (lockPath) => packageNameFromLockPath(lockPath) === "nanoid",
  );
  if (nanoidPaths.length !== 1 || nanoidPaths[0] !== "node_modules/nanoid") {
    errors.push("nanoid must resolve exactly once at node_modules/nanoid");
  }

  const nanoidEntry = packageLock.packages["node_modules/nanoid"];
  if (!isPlainObject(nanoidEntry)) {
    errors.push("missing lock entry node_modules/nanoid");
  } else if (nanoidEntry.version !== "3.3.18") {
    errors.push("nanoid lock version must be exactly 3.3.18");
  }
...
function validateApprovedAuditChain({
  ...
  const coveredNames = new Set(exception.coveredFindings);
  for (const coveredName of coveredNames) {
    if (!highNames.has(coveredName)) {
      errors.push(
        `approved covered finding ${coveredName} must have a high severity`,
      );
    }
  }
  for (const { name, entry, severities } of highOrCriticalEntries) {
    const chainEntry = CHAIN_BY_NAME.get(name);
    if (!chainEntry || !coveredNames.has(name)) {
      errors.push(`unapproved high/critical package ${name}`);
      continue;
    }
    if (entry.severity !== exception.severity) {
      errors.push(
        `${name} is ${entry.severity}; the approved exception requires ${exception.severity}`,
      );
    }
    ...
    try {
      ...
      for (const advisory of advisories) {
        ...
        if (advisoryId !== exception.advisory.toUpperCase()) {
          errors.push(
            `${name} resolves to an advisory other than ${exception.advisory}`,
          );
        }
        if (advisory.name !== exception.package) {
          errors.push(`approved advisory package must be ${exception.package}`);
        }
        if (advisory.dependency !== exception.package) {
          errors.push(
            `approved advisory dependency must be ${exception.package}`,
          );
        }
        if (advisory.severity !== exception.severity) {
          errors.push(
            `approved advisory severity must remain ${exception.severity}`,
          );
        }
        if (advisory.range !== exception.affectedRange) {
          errors.push(
            `approved advisory affected range must remain ${exception.affectedRange}`,
          );
        }
      }
    } catch (error) {
  ...
  } else if (leafEntry.range !== exception.affectedRange) {
    errors.push(
      `approved deepmerge-ts finding affected range must remain ${exception.affectedRange}`,
    );
  }

  const leafLockEntry = packageLock?.packages?.["node_modules/deepmerge-ts"];
  if (
    !isPlainObject(leafLockEntry) ||
    leafLockEntry.version !== exception.currentVersion
  ) {
    errors.push(
      `approved deepmerge-ts finding version must remain ${exception.currentVersion}`,
    );
  }
...
export function evaluateAuditReport({
  ...
  const nanoidValidation = validateNanoidResolution(packageLock);
  const initialErrors = [
    ...auditValidation.errors,
    ...exceptionValidation.errors,
    ...nanoidValidation.errors,
  ];
...
function printExceptionLog(exception) {
  process.stdout.write(
    [
      ...
      `coveredFindings: ${exception.coveredFindings.join(", ")}`,
      `reason: ${exception.reason}`,
      `ownerRole: ${exception.ownerRole}`,
      `reviewDate: ${exception.reviewDate}`,
      `reviewCondition: ${exception.reviewCondition}`,
